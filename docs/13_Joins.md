...
-   Результат соединения с явным условием соединения в предложении `ON`
    будет содержать каждый из столбцов дважды: один раз для левой
    таблицы и один раз для правой. Очевидно, что они будут иметь они и
    те же значения;
...
`a` и `ins_date` , а два следующих оператора дадут один и тот же
результат:
...
соединения, указав `LEFT`, `RIGHT` или `FULL` перед ключевым словом
`JOIN`.
...
разделяющими запятыми в предложении `FROM`. Условия соединения задаются
в предложении `WHERE` среди других условий поиска. Такие соединения
называются *неявными*.
...
Условий при этом никаких нет. Можно добавить условия в части `WHERE` по
аналогии с неявным соединением.
